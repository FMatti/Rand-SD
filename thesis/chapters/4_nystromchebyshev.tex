...
The initial idea from \cite{lin2017randomized} was to introduce a low-rank
approximation in the stochastic trace estimation to achieve a
...
\begin{theorem}{Variance reduced trace estimation}{4-nystromchebyshev-trace-correction}
    ...
    For fixed constants $c, C$, if \gls{num-hutchinson-queries} $> c\log(1/\delta)$, then with probability $\geq 1 - \delta$,
    \begin{equation}
...
the trace of a matrix $\mtx{B}(t)$ which depends continuously on a parameter $t$
...
on a certain number \gls{sketch-size} $\geq 1$, usually the \glsfirst{sketch-size}
...
\begin{theorem}{Variance reduced parameter-dependent trace estimation}{4-nystromchebyshev-trace-correction-parameter-dependent}
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}
    Then, for a fixed constant $C$ and $\delta \in (0, e^{-1})$, with probability $\geq 1 - \delta$,
    \begin{equation}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \int_{a}^{b} |\Tr(\mtx{\Delta}(t)) - \Hutch_{n_{\Psi}}(\mtx{\Delta}(t))| \mathrm{d}t && \text{(definition $\Tr^{++}$)}  \notag \\
        &= C_{\Psi} \frac{\log(1/\delta)}{\sqrt{n_{\Psi}}} \int_{a}^{b} \lVert \mtx{\Delta}(t) \rVert _F \mathrm{d}t && \text{(using \reflem{lem:2-chebyshev-parameter-hutchinson})} \notag \\
        &= C_{\Psi} C_{\Omega} \frac{\log(1/\delta)}{\sqrt{n_{\Psi} n_{\Omega}}} \int_{a}^{b} \Tr(\mtx{B}(t)) \mathrm{d}t && \text{(assumption $\mtx{\Delta}(t)$)} \notag \\
    \end{align*}
    Taking $C=C_{\Psi} C_{\Omega}$, we get the desired result with probability $\geq 1 - \delta$.
\end{proof}

When compared to the constant matrix case (\refthm{thm:4-nystromchebyshev-trace-correction}),
in the parameter-dependent case (\refthm{thm:4-nystromchebyshev-trace-correction-parameter-dependent})
...
\begin{theorem}{$L^1$-error of the parameter-dependent Nystr\"om++ trace estimator}{4-nystromchebyshev-final}
    ...
    satisfies for any symmetric positive definite matrix $\mtx{B}(t)$ which continuously
...
Compared to the constant matrix case (\refthm{thm:4-nystromchebyshev-nystrom-pp}),
we have an additional factor of $\sqrt{\log(1/\delta)}$ for the choice of
...
from the Nystr\"om approximation, which is estimated using the Hutchinson's estimator
...
\begin{equation}
    ...
    \frac{1}{n_{\Psi}}\Tr\big(
        (\underbrace{\mtx{\Psi}^{\top} g_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A}) \mtx{\Omega}}_{=\mtx{L}_2(t)^{\top}})
        (\underbrace{\mtx{\Omega}^{\top} g_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A}) \mtx{\Omega}}_{=\mtx{K}_1(t)})^{\dagger}
        (\underbrace{\mtx{\Omega}^{\top} g_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A}) \mtx{\Psi}}_{=\mtx{L}_2(t)})
      ).
\end{equation}
Notice how the involved matrices $\mtx{L}_2(t) \in \mathbb{R}^{n_{\Omega} \times n_{\Psi}}$ and $\mtx{K}_1(t) \in \mathbb{R}^{n_{\Omega} \times n_{\Omega}}$
...
\end{equation}
(which is, unlike suggested in \cite[algorithm~4]{lin2017randomized},
not the same as the generalized eigenvector from \refequ{equ:3-nystrom-low-rank-eigenvalue-problem})
we can compute
\begin{equation}
...
to form $\widehat{\phi}_{\sigma}(t)$ from the Nystr\"om approximation of
...
it is therefore necessary to compute the correction term $\Hutch_{n_{\Psi}}(\widehat{g}_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A}))$
using the same Hutchinson's estimator which is already used to compute the
...
\begin{align*}
    ...
    &= \mathbb{E}\bigg[\frac{1}{n_{\Psi}} \Tr( (\underbrace{\mtx{\Psi}^{\top} g_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A}) \mtx{\Omega}}_{=\mtx{L}_2(t)^{\top}})
                                               (\mtx{D}(t) \mtx{D}(t)^{\top})
                                               (\underbrace{\mtx{\Omega}^{\top} g_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A}) \mtx{\Psi}}_{=\mtx{L}_2(t)})) \bigg].
...
From \refthm{thm:4-nystromchebyshev-nystrom-pp} it is possible
...
a result similar to \refthm{thm:2-delta-gauss-chebyshev} for the \gls{DGC} method is out of reach for now.
