...
\begin{theorem}{Variance reduced parameter-dependent trace estimation}{4-nystromchebyshev-trace-correction-parameter-dependent}
    ...
    $\mtx{\Delta}(t)$ be any
    symmetric \gls{PSD} and continuous matrices in $t \in [a, b]$ such that
    \begin{equation}
        \begin{cases}
        ...
        \end{cases}
    \end{equation}
    Then, for a fixed constant $C$ and with probability $\geq 1 - \delta$, it holds
    \begin{equation}
        \int_{a}^{b} |\Tr^{++}(\mtx{B}(t)) - \Tr(\mtx{B}(t))| \mathrm{d}t \leq C \sqrt{\frac{\log(2/\delta)}{n_{\Psi} n_{\Omega}}} \int_{a}^{b} \Tr(\mtx{B}(t)) \mathrm{d}t.
    \end{equation}
    In particular, if \gls{sketch-size} $=$ \gls{num-hutchinson-queries} $=\mathcal{O}\left( \sqrt{\log(2/\delta)} / \varepsilon \right)$,
...
We also require an additional factor of $2$ inside the logarithm when choosing
...
we have an additional factor of $2$ inside the logarithm for the choice of
\gls{sketch-size} and \gls{num-hutchinson-queries}. \todo{other differences}
...
\todo{improve this discussion}
An interesting observation, which we can take from \refequ{equ:3-nystrom-converted-generalized-eigenvalue-problem},
...
\end{equation}
in other words, the matrix carrying the approximated eigenvalues of
$g_{\sigma}^{(m)}(t\mtx{I} - \mtx{A})$ on its diagonal.
