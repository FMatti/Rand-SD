\chapter{Improved trace estimation}
...
and \refchp{chp:3-nystrom} combine them to an improved trace estimatior which
...
Hutchinson's estimator will trivially be exact. Thus, \refequ{equ:4-nystromchebyshev-hutch-pp}
...
the form \refequ{equ:3-nystrom-RSVD} to prove the $\mathcal{O}(\epsilon^{-1})$-dependence
...
\begin{theorem}{$L^1$-error of Nystr\"om-Chebyshev++ method}{4-nystromchebyshev-nystromchebyshev-method}
    ...
    $C_1, C_2$ in \reflem{lem:2-chebyshev-error}, then
...
\begin{proof}
    ...
    \begin{align*}
        ...
        %&\leq \varepsilon \frac{C_1}{\sigma^2} (1 + C_2 \sigma)^{-m} + \varepsilon (1 + 2 \rho) 
        %+ \frac{C_1}{\sigma^2} (1 + C_2 \sigma)^{-m} && \text{(\reflem{lem:2-chebyshev-error} and $\lVert \phi_{\sigma} \rVert _1=1$)} \notag \\
...
However, the non-linearity of most low-rank factorizations make the estimator
$\Tr^{++}$ non-linear, and disallow extending this error guarantee to
$\lVert \phi_{\sigma} - \breve{\phi}_{\sigma}^{(m)} \rVert$.
