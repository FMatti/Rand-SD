...
\end{equation}
exhibits an approximate low-rank structure, unless of course \gls{smoothing-parameter} is chosen exceedingly big or all eigenvalues
are clustered around \gls{spectral-parameter}. The matrix can approximately 
...
This approximation is referred to as the Nystr\"om approximation \cite{gittens2013nystrom}
\begin{equation}
...
\begin{theorem}{Frobenius norm error of Nystr\"om approximation}{3-nystrom-nystrom-constant}
    Let $\mtx{B}$ be a symmetric \gls{PSD} matrix. Its Nystr\"om approximation
...
for all $t$. This will introduce massive gains in terms of computational
...
a much stronger result can be shown for matrices $\mtx{B}(t)$ whose eigenvalues
...
distance from \gls{spectral-parameter}, such that $\lambda_1(g_{\sigma}(t\mtx{I} - \mtx{A})) \geq \dots \geq \lambda_n(g_{\sigma}(t\mtx{I} - \mtx{A}))$.
...
and $\mtx{K}_2(t) \in \mathbb{R}^{n_{\Omega} \times n_{\Omega}}$ efficiently.
...
We usually use \gls{short-circuit-threshold} $= 10^{-5}$. The effect this short-circuit
...
\begin{figure}[ht]
    ...
    \caption{The difference the short-circuit mechanism can make when approximating
        ...
        and ran the \gls{NC} method with \glsfirst{short-circuit-threshold} $> 0$,
...
It allows us to convert \refequ{equ:3-nystrom-low-rank-eigenvalue-problem}
...
by the eigenvectors corresponding to sufficiently large eigenvalues of
$\mtx{\Omega}^{\top} \mtx{B} \mtx{\Omega}$.\\
%By identifying 
...
\begin{equation}
    \lVert \phi_{\sigma} - \widehat{\phi}_{\sigma} \rVert _1 \leq \gamma\sqrt{1+r} \left\lVert \sum_{i=r+1}^{n} g_{\sigma}(\cdot - \lambda_{(i)}) \right\rVert _1,
...
on the left-hand side of \refequ{equ:3-nystrom-spectral-density-error}
will quickly tend towards zero, hence guaranteeing us a good approximation of
...
A comparison of the methods can be found in \reffig{fig:3-nystrom-other-approximations}
