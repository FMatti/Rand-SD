...
Hence, for a symmetric matrix $\mtx{B} \in \mathbb{R}^{n \times n}$
...
with a standard Gaussian \gls{sketching-matrix} $\in \mathbb{R}^{n \times n_{\mtx{\Omega}}}$,
...
\begin{proof}
    \todo{Check if this goes through} For every $t \in [a, b]$ we have
    \begin{align}
...
numerical rank of the matrix not accurate in this case, since the spectrum is
...
\begin{figure}[ht]
    ...
    \caption{Singular value decay of a Gaussian \glsfirst{smoothing-kernel}
       ...
       \refsec{sec:5-experiments-density-function} for $c=1$.
       \todo{Add spectrum to show uneven distribution}}
...
For some standard Gaussian random matrix $\mtx{\Omega} \in \mathbb{R}^{n \times n_{\mtx{\Omega}}}$,
...
computes the $n_{\mtx{\Omega}}$-query Hutchinson's estimate \refequ{equ:2-chebyshev-DGC-hutchionson-estimator}
\begin{equation}
    \frac{1}{n_{\mtx{\Omega}}} \Tr(\mtx{K}_1(t_i))
...
If the result is smaller than a threshold $\delta > 0$, it immediately sets
$\widetilde{\phi}_{\sigma}^m(t_i)=0$ and skips \reflin{lin:3-nystrom-pseudo-inverse}.
for this $t_i$. We usually use $\delta = 10^{-5}$. The effect this short-circuit
...
its elements should be within the range of $g_{\sigma}$. Hence, we may remove
...
[Numerical experiments comparing resolution approaches]\\
}

Finally, we can summarize this alternative way of treating
