...
Nystr\"om approximation in the Frobenius norm:

\begin{theorem}{Accuracy of Nystr\"om approximation}{3-nystrom-nystrom-constant}
    \begin{equation}
        \lVert \mtx{B} - \widehat{\mtx{B}} \rVert _F \leq \todo{?}%\lVert \mtx{\Sigma}_2 \rVert _F + \lVert \mtx{\Sigma}_2^{1/2} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert ^2_F
    \end{equation}
\end{theorem}
...
\todo{Probably need to remove this}
\begin{proof}
...
a consistent expansion for \gls{smoothing-kernel}$^2$ which is just as fast.
\todo{(compare timing with Lin Lin)}
...
\todo{maybe add plots or comparison (or maybe only in chapter 5)}
