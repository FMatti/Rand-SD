...
%            &= \lVert (\mtx{I}_n - \Pi_{\mtx{B}(t)^{1/2} \mtx{\Omega}})\mtx{B}(t)^{1/2} \rVert _F^2 && \text{(\cite[proof of corollary 8.8]{tropp2023randomized})}
%    \end{align}
...
\paragraph{Pseudo-inverse via eigenvalue problem}
...
\begin{figure}[ht]
    ...
    \caption{The difference the filter tolerance can make when approximating
        a spectral density using \refalg{alg:3-nystrom-nystrom-chebyshev}. 
        Here, we used the matrix described in \refsec{sec:5-experiments-density-function}
        and ran the \gls{NC} method with and without \glsfirst{filter-tolerance}
...
%Using \refthm{thm:3-nystrom-frobenius-norm} we can now conclude that for some $\gamma \geq 1$, with probability $\geq 1-\gamma^{-p}$,
%\begin{equation}
%    \lVert \phi_{\sigma} - \widehat{\phi}_{\sigma} \rVert _1 \leq \gamma^2(1+r) \int_{-1}^{1} \sum_{i=r+1}^{n} g_{\sigma}(t - \lambda_{(i)}) \mathrm{d}t,
...
%a general result similar to \refthm{thm:2-delta-gauss-chebyshev} does not immediately follow.
%However, if we know that the Chebyshev expansion $g_{\sigma}^{(m)}$ does not
%deviate by more than $\rho$ from \gls{smoothing-kernel}, then we can guarantee
%that the function $\underline{g}_{\sigma}^{(m)} = g_{\sigma}^{(m)} + \rho$ is
%non-negative, and therefore, $\underline{g}_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A})$
%\gls{PSD}. This allows us to apply \refthm{thm:3-nystrom-frobenius-norm} to the
%resulting up-shifted spectral density approximation the \gls{NC} method would
%compute, which approaches \refequ{equ:3-nystrom-spectral-density-error} as the
...
However, due to \refthm{lem:2-chebyshev-error}, we know that for large
...
we may retrieve \glsfirst{smooth-spectral-density} using the linearity of the trace
\footnote{For the \gls{NC} method, this correction will have to be adjust to
...
\begin{theorem}{$L^1$-error of Nystr\"om-Chebyshev method with shift}{3-nystrom-nystrom-method}
    ...
    $C_1, C_2$ in \reflem{lem:2-chebyshev-error}
...
\begin{proof}
    ...
    %    &\leq \lVert \widehat{\underline{\phi}}_{\sigma}^{(m)} - \underline{\phi}_{\sigma}^{(m)} \rVert _1
    %    + \lVert \underline{\phi}_{\sigma}^{(m)} - \underline{\phi}_{\sigma} \rVert _1 && \text{(triangle inequality)} \notag \\
    %    &\leq \gamma^2(1 + r) \int_{-1}^{1} \sum_{i=r+1}^{n} \sigma_i(\underline{g}_{\sigma}^{(m)}(t\mtx{I}_n - \mtx{A})) \mathrm{d}t
    %    + \frac{C_1}{\sigma^2} (1 + C_2 \sigma)^{-m} && \text{(\refthm{thm:3-nystrom-frobenius-norm} and \reflem{lem:2-chebyshev-error})} \notag \\
    %    %&\leq \gamma^2(1 + r) \int_{-1}^{1} \sum_{i=r+1}^{n} (g_{\sigma}(t - \lambda_{(i)}) + 2\rho) \mathrm{d}t
    %    %+ \frac{C_1}{\sigma^2} (1 + C_2 \sigma)^{-m} && \text{(\textcolor{red}{TODO})} \notag \\
    %    %&\leq \gamma^2(1 + r) \int_{-1}^{1} \sum_{i=r+1}^{n} g_{\sigma}(t - \lambda_{(i)})\mathrm{d}t + \gamma^2(1 + r)4(n-r)\rho
    %    %+ \frac{C_1}{\sigma^2} (1 + C_2 \sigma)^{-m} && \text{(\textcolor{red}{TODO})} \notag \\
    %\end{align*}
    %Whatever approximation, we may thu
    \begin{equation}
        \lVert \underline{\phi}_{\sigma} - \widehat{\underline{\phi}}_{\sigma}^{(m)}  \rVert _1
    ...
    under constant shifts, we deduce with \refthm{lem:2-chebyshev-error} that the
    ...
    \begin{align*}
        ...
        %&\leq \gamma^2(1 + r) \int_{-1}^{1} \sum_{i=r+1}^{n} \sigma_i(g_{\sigma}(t\mtx{I}_n - \mtx{A})) + \rho(n^{-1} + 1) \mathrm{d}t \notag \\
...
evenly distributed, we know from \refsec{sec:3-nystrom-numerical-rank}
that
\begin{equation}
...
From \refequ{fig:3-nystrom-shift} it is clear that adding the shift harms the
convergence of the approximation. Therefore, this shifting operation is only
useful to guarantee theoretical result, but in practice it seems like the
...
%in \refthm{thm:3-nystrom-nystrom-method}
