...
the convergence of the error with \gls{num-random-vectors} in \reffig{fig:5-experiments-electronic-structure-convergence-nv}
...
%\begin{equation}
%    \Tr\left[((t + i\sigma)I - A)^{-1}\right] \approx \frac{1}{n_v} \sum_{j=1}^{n_v} \left( \vct{\psi}_j \right)^{\top} ((t - i\sigma)\mtx{I} - \mtx{A})^{-1} \vct{\psi}_j.
