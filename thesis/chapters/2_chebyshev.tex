...
\begin{lemma}{$L^1$-error of Chebyshev expansion for Gaussian smoothing}{2-chebyshev-error}
    ...
    is contained in $[-1, 1]$. Then the expansion of the Gaussian \glsfirst{smoothing-kernel}
    and the corresponding \glsfirst{smooth-spectral-density} satisfy
    \begin{align}
...
\begin{proof}
    ...
    and the analyticity of \gls{smoothing-kernel} we know that
    \begin{equation}
        \sup_{s \in [-1, 1]} \left| g_{\sigma}(t - s) - g_{\sigma}^{(m)}(t - s) \right|
    ...
    \begin{equation}
        \left| g_{\sigma}(t) - g_{\sigma}^{(m)}(t) \right|
    ...
    allows us to also show the last assertion with
    \begin{equation}
