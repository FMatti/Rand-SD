...
contained in $[-1, 1]$. To also apply the \gls{DGC} method to matrices whose
spectra we know (or can estimate) to be within $[a, b] \subset \mathbb{R}$, we can define a
...
\end{equation}
Then, we can apply the \gls{DGC} method to $\bar{\mtx{A}} = \tau(\mtx{A})$ whose
spectrum is contained in $[-1, 1]$. In order to be able to retrieve the spectral density
for the original matrix $\mtx{A}$, we will have to use the rescaled
\begin{equation}
...
In all our examples, we will implicitly assume the rescaling
\refequ{equ:2-chebyshev-sigma-transformation} has been done.\\
