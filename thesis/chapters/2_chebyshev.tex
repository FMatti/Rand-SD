...
against the equivalent algorithm from \cite{lin2017randomized}. The results
...
of the recurrence can cost up to $\mathcal{O}(n^3)$, and the evaluation at
\gls{num-evaluation-points} values of \gls{spectral-parameter} could furthermore
cost $mathcal{O}(n^2 m n_t)$ operations. Therefore, in case we are only interested
in applying a linear mapping to the interpolant, a significant speed-up can be
...
The convergence of the expansion to the actual \gls{smoothing-kernel} is exponential
...
\begin{theorem}{Convergence of Delta-Gauss-Chebyshev method}{2-delta-gauss-chebyshev}
