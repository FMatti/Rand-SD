...
have found a significantly simpler, faster method, and provably accurate method:
...
the interpolant. In \reffig{fig:2-chebyshev-sketched-interpolation} some examples
...
in \refalg{alg:2-chebyshev-DGC} from $\mathcal{n^2n_{\Psi}}$ to $\mathcal{nn_{\Psi}}$.
...
\begin{proof}
    ...
    \begin{align}
        \lVert \phi_{\sigma}^m(t) - \widetilde{\phi}_{\sigma}^m(t) \rVert _1
            &= \lVert \Tr(g_{\sigma}^m(t\mtx{I} - \mtx{A})) - \Hutch_{n_{\Psi}}(g_{\sigma}^m(t\mtx{I} - \mtx{A})) \rVert _1 && \text{(definitions)} \notag \\
            &\leq C_{\Psi} \sqrt{\frac{\log(2/\delta)}{n_{\Psi}}} \int_{-1}^{1} \lVert g_{\sigma}^m(t\mtx{I} - \mtx{A}) \rVert _F \mathrm{d}t && \text{(\reflem{lem:2-chebyshev-parameter-hutchinson})}
    \end{align}
    %\begin{align}
    ...
    \begin{align}
        ...
        &\leq \lVert g_{\sigma}^m(t\mtx{I} - \mtx{A}) - g_{\sigma}(t\mtx{I} - \mtx{A}) \rVert _F + \lVert g_{\sigma}(t\mtx{I} - \mtx{A}) \rVert _F && \text{(triangle inequality)} \notag \\
        &\leq \sqrt{n} \sup_{s \in [-1, 1]} | g_{\sigma}^m(s) - g_{\sigma}(s) | + \Tr(g_{\sigma}(t\mtx{I} - \mtx{A})) && \text{(pessimistic upper bounds)} \notag \\
