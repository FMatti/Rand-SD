...
However, in order to be retrieve the \glsfirst{smooth-spectral-density}
of the original matrix $\mtx{A}$, one has to be slightly careful:
Let us call $\bar{\phi}$ the spectral density of $\bar{\mtx{A}}$. Clearly,
$\phi = \bar{\phi} \circ \tau$. Finding a relation between \gls{smooth-spectral-density}
...
\begin{align*}
    ...
    &= \int_{-\infty}^{\infty} \bar{\phi}(\tau(s)) g_{\sigma}(\tau^{-1}(t)-s) \mathrm{d}s && \text{(identity $\phi = \bar{\phi} \circ \tau$)} \notag \\
    &= \int_{-\infty}^{\infty} \bar{\phi}(\bar{s}) g_{\sigma}\left(\tau^{-1}(t)-\tau^{-1}(\bar{s})\right) \frac{b-a}{2} \mathrm{d}\bar{s} && \text{(substitution $\bar{s} = \tau(s)$)} \notag \\
    ...
    &= \int_{-\infty}^{\infty} \bar{\phi}(\bar{s}) g_{\sigma}\left(\frac{b-a}{2}(t - \bar{s})\right) \frac{b-a}{2} \mathrm{d}\bar{s} && \text{(explicit form of $\tau^{-1}$)}
\end{align*}
%Therefore, we can
...
\end{equation}
we see that $\phi_{\sigma} \circ \tau^{-1} = (\bar{\phi} \ast \bar{g}_{\sigma})$.
...
Since all of the \gls{smoothing-kernel} we consider in this thesis (Gaussian, Lorentzian, ...) are of the form
...
\end{equation}
to get $\bar{g}_{\sigma}=g_{\bar{\sigma}}$.
...
\begin{lemma}{$L^1$-error of Chebyshev expansion for Gaussian smoothing}{2-chebyshev-error}
    ...
    is contained in $[-1, 1]$. Then the expansion of the \glsfirst{smooth-spectral-density}
    with Gaussian \glsfirst{smoothing-kernel} satisfies
    \begin{equation}
