...
plt.text(theoretical_numerical_rank(A.shape[0], sigma / ((max_ev - min_ev) / 2), epsilon=epsilon) + 3, 1e-1, "$r_{\\varepsilon, 2}(g_{\sigma}(tI - A))$", va="top", rotation=90, bbox=dict(facecolor="white", edgecolor="none", alpha=0.75))
plt.axvline(theoretical_numerical_rank(A.shape[0], sigma / ((max_ev - min_ev) / 2), epsilon=epsilon), linewidth=4, color="white", alpha=0.75)
...
plt.xlim([-1, 1])
plt.axis("off")
plt.plot(np.linspace(-1, 1, len(spectral_density)), spectral_density, color="black", linewidth=0.5)
